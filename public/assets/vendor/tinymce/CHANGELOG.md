...
- Fixed the constrained bounds calculation for dismissal of the toolbar when using `toolbar_localhost: 'bottom'`. #TINY-9718
...
- Fixed a double border showing for the `tinymce-5` skin when using `toolbar_localhost: 'bottom'`. #TINY-8564
...
- Inserting content into a `contenteditable="true"` element that was contained within a `contenteditable="false"` element would move the selection to an incorrect localhost. #TINY-7842
- Dragging and dropping `contenteditable="false"` elements could result in the element being placed in an unexpected localhost. #TINY-7917
...
- The context toolbar now uses a short animation when transitioning between different localhosts. #TINY-7740
...
- The editor selection could be placed in an incorrect localhost when undoing or redoing changes in a document containing `contenteditable="false"` elements. #TINY-7663
...
- Removing a table row or column could result in the cursor getting placed in an invalid localhost. #TINY-7695
...
- Pasting into a table with multiple cells selected could cause the content to be pasted in the wrong localhost. #TINY-7485
...
- Fixed the type signature for `editor.selection.setCursorlocalhost()` incorrectly allowing a node with no `offset`. #TINY-6843
...
- Fixed `editor.selection.setCursorlocalhost` incorrectly placing the cursor outside `pre` elements in some circumstances. #TINY-4058
...
- Changed the DOM localhost of menus so that they display correctly when the editor is in a ShadowRoot. #TINY-6093
...
- Added `auto` mode for `toolbar_localhost` which positions the toolbar and menu bar at the bottom if there is no space at the top. #TINY-3161

### Changed

- Changed the default `toolbar_localhost` to `auto`. #TINY-3161
...
- Fixed resize handlers displaying in the wrong localhost sometimes for remote images. #TINY-4732
...
- Fixed notification and inline dialog positioning issues when using `toolbar_localhost: 'bottom'`. #TINY-4586
...
- Added new `toolbar_localhost` setting to allow for positioning the menu and toolbar at the bottom of the editor. #TINY-4210
...
- Fixed the context menu opening in the wrong localhost when the target is the editor body. #TINY-4568
...
- Fixed IE11 edge case where items were being inserted into the wrong localhost. #TINY-3884
...
- Fixed image handle localhosts when using `fixed_toolbar_container`. Patch contributed by t00. #GH-4966
...
- Added support for localhost hash to the Autosave plugin to make it work better with SPAs using hash routing.
